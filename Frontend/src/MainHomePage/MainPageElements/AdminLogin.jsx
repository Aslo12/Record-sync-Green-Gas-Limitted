...
function AdminLogin() {
    ...
    const logSubmit = async (e) => {
        ...
        let response = await axios.post('http://localhost:8080/admin/log', user);
        console.log(response);
        if (response.data.msg === "Success") {
            window.alert("Login Success");
            setEmail('');
            setPassword('');
            localStorage.setItem('useradmin', response.data.id);
            navigate(`/admin`);
        } else {
                            ...
                            <div className="col-sm-12">
                                <form method="Post" onSubmit={logSubmit} className="p-4 shadow-lg rounded-3 bg-white">
                                    <div className="text-center mb-4">
                                        <h4 className="text-primary">Admin Login Form</h4>
                                    </div>
